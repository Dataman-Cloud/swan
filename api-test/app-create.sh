...
then
  ...
  for app in `echo ${apps} | jq '.apps[].name' | tr -d '"'`; do
...
then
  ...
  ok "create app with same name should fail"
fi
...
appsLen=`http --check-status --ignore-stdin --timeout=4.5 get $SERVER_PATH/$PATH_PREFIX/apps | jq '.apps | length'`
...
if [ "$?" != "4" ]
then
  fail "create app with same name `cat @template-replicates.json | jq .name` should fail"
else
  ok "create app with same name should fail"
fi
