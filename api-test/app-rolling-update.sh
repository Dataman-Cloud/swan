...
task0Version=`http get http://localhost:9999/v_beta/apps/nginx0051  | jq '.app.tasks | .[] | select( .id | contains("0-")) .versionId'`
task1Version=`http get http://localhost:9999/v_beta/apps/nginx0051  | jq '.app.tasks | .[] | select( .id | contains("1-")) .versionId'`
appVersion=`http get http://localhost:9999/v_beta/apps/nginx0051  | jq '.app .versions | .[]'`
