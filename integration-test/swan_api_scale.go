...
SCALEUP:
	scale := new(types.ScalePolicy)
	scale.Instances = 30
	scale.Step = 2
	scale.OnFailure = "continue"

	data, _ = s.encode(scale)
	fmt.Print("POST http://", s.SwanHost, "/v1/apps", rt.Id, "/scale")
	code, body, err = s.sendRequest("POST", "/v1/apps/"+rt.Id+"/scale", bytes.NewReader(data))
	c.Assert(err, check.IsNil)
	c.Assert(code, check.Equals, http.StatusAccepted)
	c.Assert(string(body), check.Equals, "accepted")
	fmt.Println(" ", code)

	fmt.Println("Waiting app", rt.Id, "to scale up to 30")
	for {
		select {
		case <-time.After(5 * time.Minute):
			c.Errorf("task count not exceed %d after 5 minute", 30)
			return
		case <-ticker.C:
			fmt.Print(".")
			code, body, err := s.sendRequest("GET", "/v1/apps/"+rt.Id, nil)
...
SCALEDOWN:
	...
	fmt.Println("Waiting app", rt.Id, "to scale down to 10")
	for {
		select {
		case <-time.After(5 * time.Minute):
			c.Errorf("task count not exceed %d after 5 minute", 30)
