...
type Selector interface {
	...
	// Empty returns true if this selector does not restrict the space.
	Empty() bool
...
// If any these rules is violated, and error is returned.
// 1. The operator can only be In, NotIn, Equals, DoubleEquals, NotEquals, Exists or DoesNotExist.
// 2. If the operator is In or NotIn, the values set must be non-empty.
// 3. If the operator is Equals, doubleEqualsa, or NorEquals the values set must contain one value.
// 4. If the operator is Exists or DoesNotExist the value set must be empty
// 5. If the operator is Gt or Lt, the values set mut contain only value, which will be interpreted an integer.
	...
	case selection.GreaterThan, selection.LessThan:
		...
		for i := range vals {
			if _, err := strconv.ParseInt(vals[i], 10, 64); err != nil {
				return nil, fmt.Errorf("for 'Gt', 'Lt' operator, the value must be an integer")
			}
		}
...
// Matches returns true if the Requiremnt matched the input Labels.
// There is a match in the following case:
// 1. The operator is Exist and Labels has the Requirement's key.
// 2. The operator is In, Labels has the Requirements key and Labels value
//    for that key is in Requirement's value set.
// 3. The operator is NotIn, Labels has the Requirement's key and Labels's
//    value for that key is not in Requirement's value set
...
func (r *Requirement) Matches(ls Labels) bool {
	...
	case selection.GreaterThan, selection.LessThan:
		...
		if err != nil {
			logrus.Infof("ParseInt failed for value %+v in label %+v %+v", ls.Get(r.key), ls, err)
			return false
		}

		if len(r.strValues) != 1 {
			logrus.Infof("Invalid values count %+v of requirement %#v, for 'Gt', 'lt' exactly one value is required", len(r.strValues), r)
			return false
		}

		rValue, err := strconv.ParseInt(r.strValues[0], 10, 64)
		if err != nil {
			logrus.Infof("ParseInt failed for value %+v in requirement %#v, for 'Gt', 'Lt' operators, the value must be an integer", r.strValues[0], r)
			return false
		}
...
func (r *Requirement) Values() map[string]struct{} {
	...
	for i := range r.strValues {
		ret[r.strValues[i]] = struct{}{}
	}

	return ret
}

// Returns true if internalSelector doesn't restrict space
...
// Add adds requirements to the selector. It copies the current selector returnning a new one
...
// its Requirement match the input Labels. If any Requirement
...
// String returns a comma-separted string of all
// the internalSelector Requirements's human-readable stings
...
func (p *Parser) parseRequirement() (*Requirement, error) {
	...
	return NewRequirement(key, operator, valueList)
...
func (p *Parser) parseValues() (map[string]struct{}, error) {
	...
	case ClosedParToken: // handles "()"
		p.consume(Values)
		return map[string]struct{}{"": struct{}{}}, nil
	default:
...
func (p *Parser) parseExactValue() (map[string]struct{}, error) {
	s := make(map[string]struct{})
	tok, lit := p.lookahead(Values)
